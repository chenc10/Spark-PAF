  ...
  extends TaskScheduler with Logging
{
  ...
  // add by cc
  private val executorIdToTaskSetManager = new HashMap[String, TaskSetManager]
  private val executorIdToTid = new HashMap[String, Long]
  private val executorIdToSign = new HashMap[String, Int]
  private val executorIdToLastIdleTime = new HashMap[String, Long]
  private val c_taskIdToExecutorId = new HashMap[Long, String]
  //
  private val waivedThreadIdList = new ArrayBuffer[Int]
  //
  private var cummulativeWasteTime = 0L
  private var cummulativeWasteTimeRigid = 0L
//  private val executorIdToStartWaitingTime = new HashMap[String, Long]
//  private val executorIdToThreshold = new HashMap[String, Long]
  private val handledJobList = new ArrayBuffer[Int]
    ...
    for (i <- 0 until shuffledOffers.size) {
      ...
      if (!executorIdToTaskSetManager.contains(execId)){
        logInfo("##### ##### ##### new executor! - %s".format(execId))
          ...
          if (taskSet.threadId == executorIdToTaskSetManager(execId).threadId){
...
//              if (taskSet.priority < executorIdToTaskSetManager(execId).priority) {
              if (handledJobList.contains(executorIdToTaskSetManager(execId).jobId)) {
                ...
                if (waivedThreadIdList.contains(executorIdToTaskSetManager(execId).threadId)){
                  logInfo("##### - ##### slot reservation has been waived for this thread")
                } else {
                  logInfo("##### - ##### need to enforce slot sticking")
                }
              }
          }
      }
        ...
        if (availableCpus(i) >= CPUS_PER_TASK) {
          try {
            for (task <- taskSet.resourceOffer(execId, host, maxLocality)) {
              ...
              val tid = task.taskId
              // add by cc
             // executorIdToIsWaiting(execId) = 0
              taskIdToTaskSetManager(tid) = taskSet
              taskIdToExecutorId(tid) = execId

              c_taskIdToExecutorId(tid) = execId
...
//        logInfo("##### ##### skip taskSet-%d to exec-%s".format(taskSet.jobId, execId))
      }
    }
    return launchedTask
  }
...
//    logInfo("##### ##### ##### ##### enter resourceOffers, offer size: %d".format(offers.size))
    ...
    var launchedTask = false
    handledJobList.clear()
    for (taskSet <- sortedTaskSets){
//     logInfo("###### total locality levels num: %d".format(taskSet.myLocalityLevels.size))
      for (maxLocality <- taskSet.myLocalityLevels) {
        do {
//          logInfo("###### taskSet: %d; ".format(taskSet.jobId))
          launchedTask = resourceOfferSingleTaskSet(
            taskSet, maxLocality, shuffledOffers, availableCpus, tasks)
//          logInfo("###### current locality finish -1 ")
        } while (launchedTask)
//        logInfo("###### current locality finish")
      }
      handledJobList.append(taskSet.jobId)
    }
...
//    logInfo("##### ##### ##### ##### leave resourceOffers, offer size: %d".format(offers.size))
  ...
  def statusUpdate(tid: Long, state: TaskState, serializedData: ByteBuffer) {
        ...
        taskIdToTaskSetManager.get(tid) match {
          case Some(taskSet) =>
            if (TaskState.isFinished(state)) {
              ...
              logInfo("@@@@@@@ delete tid: %d".format(tid))
    ...
    if (failedExecutor.isDefined) {
      ...
      logInfo(" ----- ----- revive here: (2) statusUpdate & failedExecutor.isDefined")
      backend.reviveOffers()
    }
  }
...
//    }
  ...
  def handleFailedTask(
    ...
    if (!taskSetManager.isZombie && taskState != TaskState.KILLED) {
      ...
      logInfo(" ----- ----- revive here: (3) handleFailedTask")
      backend.reviveOffers()
    }
  }
